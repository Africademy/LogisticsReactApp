...
import { saveTotalgrossvolume, getTotalgrossvolume } from "../../services/totalgrossvolumeService";

class createTotalgrossvolume extends Component{
...
super(props);
  ...
  async populateForm() {
    try {
      const totalgrossvolumeId = this.props.match.params.id;
      if(totalgrossvolumeId!=="new"){
        const { data } = await getTotalgrossvolume(totalgrossvolumeId);
        this.setState({ data });
      }    
    } 
    catch (ex) {
  ...
  handleSubmit = async (event) => {
    ...
    if (errors) return;
    await saveTotalgrossvolume(this.state.data);
    this.props.history.push("/totalgrossvolumes");
  };

  render() {
    return (
      <div className="content">
        <h1>Totalgrossvolume Form</h1>
...
export default createTotalgrossvolume;