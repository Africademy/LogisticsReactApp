...
import { saveHeight, getHeight } from "../../services/heightService";

class createHeight extends Component{
...
super(props);
  ...
  async populateForm() {
    try {
      const heightId = this.props.match.params.id;
      if(heightId!=="new"){
        const { data } = await getHeight(heightId);
        this.setState({ data });
      }    
    } 
    catch (ex) {
  ...
  handleSubmit = async (event) => {
    ...
    if (errors) return;
    await saveHeight(this.state.data);
    this.props.history.push("/heights");
  };
