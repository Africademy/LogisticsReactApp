...
import { saveTransportmeansid, getTransportmeansid } from "../../services/transportmeansidService";

class createTransportmeansid extends Component{
...
super(props);
  ...
  async populateForm() {
    try {
      const transportmeansidId = this.props.match.params.id;
      if(transportmeansidId!=="new"){
        const { data } = await getTransportmeansid(transportmeansidId);
        this.setState({ data });
      }    
    } 
    catch (ex) {
  ...
  handleSubmit = async (event) => {
    ...
    if (errors) return;
    await saveTransportmeansid(this.state.data);
    this.props.history.push("/transportmeansids");
  };

  render() {
    return (
      <div className="content">
        <h1>Transportmeansid Form</h1>
...
export default createTransportmeansid;